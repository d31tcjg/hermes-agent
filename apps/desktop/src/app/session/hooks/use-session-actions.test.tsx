...
function ResumeHarness({
  ...
  onReady: (resume: (storedSessionId: string, replaceRoute?: boolean, profileHint?: null | string) => Promise<unknown>) => void
  requestGateway: <T>(method: string, params?: Record<string, unknown>) => Promise<T>
}) {
  const ref = <T,>(value: T): MutableRefObject<T> => ({ current: value })
...
describe('resumeSession failure recovery', () => {
  ...
  async function runResume(
    ...
    if (!mockedGetSession.getMockImplementation()) {
      mockedGetSession.mockRejectedValue(new Error('not found'))
    }

    let resume: ((storedSessionId: string, replaceRoute?: boolean, profileHint?: null | string) => Promise<unknown>) | null = null
    render(<ResumeHarness onReady={r => (resume = r)} requestGateway={requestGateway} />)
    await waitFor(() => expect(resume).not.toBeNull())
    await resume!('stored-1', true, profile)
  }

  it('routes resume lookups and RPC through an explicit profile hint', async () => {
    vi.mocked(getSession).mockResolvedValue({ id: 'stored-1', profile: 'ubuntu-server' } as never)
